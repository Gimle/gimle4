...
//		}
		...
		$dodump = function ($var, $var_name = null, $indent = 0) use (&$dodump) {
			...
			if (is_array($var)) {
				...
				foreach ($var as $key => $value) {
					call_user_func(__CLASS__ . '::color', $value, '[\'' . $key . '\']', $indent + 1);
				}
				echo str_repeat($doDump_indent, $indent) . ')';
			}
			elseif (is_string($var)) {
				echo ' = ' . call_user_func(__CLASS__ . '::color', 'String(' . strlen($var) . ')', 'gray') . ' ' . call_user_func(__CLASS__ . '::color', '\'' . call_user_func(__CLASS__ . '::_fixDumpString', $var_name, $var) . '\'', 'green');
			}
			elseif (is_int($var)) {
		...
		if ($title === false) {
			$dodump($var, self::_getCalleeFirstParam());
		}
		else {
			$dodump($var, $title);
		}
	 ...
	 * @return string
	 */
	public static function _fixDumpString ($name, $value) {
